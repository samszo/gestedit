...
function formulaires_ajouter_traduction_charger_dist($id_rubrique){
	...
	if($trad_new) $valeurs['_hidden'] .='<input type="hidden" name="trad_new" value="'.$trad_new.'"/>';	
	return $valeurs;
	}
...
function formulaires_ajouter_traduction_traiter_dist(){
	...
	if($trad_new)sql_updateq('spip_rubriques',array('id_trad'=>$id_trad),'id_rubrique='.$id_trad);

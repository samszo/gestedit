...
/**
    ...
    public function getAllVente($resume=true)
    {
        ...
        , COUNT(DISTINCT r.id_royalty) nbRD, COUNT(r.id_royalty) nbR
        , count(distinct v.id_vente) nb_idventeD , count(v.id_vente) nb_idvente, SUM(v.nombre) nb_vente 
        , SUM(v.montant_euro) mt_e , SUM(v.montant_livre) mt_l, SUM(v.montant_dollar) mt_d        
		, GROUP_CONCAT(DISTINCT(IFNULL(b.nom,''))) boutiques			
        , MAX(v.date_vente) date_last , MIN(v.date_vente) date_first			
        , SUM(r.montant_livre) mt_rTot
        ...
        FROM iste_auteur a 
			...
			INNER JOIN iste_royalty r ON ac.id_auteurxcontrat = r.id_auteurxcontrat
            INNER JOIN iste_vente v ON v.id_vente = r.id_vente
            INNER JOIN iste_boutique b ON b.id_boutique = v.id_boutique
            LEFT JOIN iste_royalty rDue  ON rDue.id_royalty = r.id_royalty AND rDue.date_paiement is null 
            LEFT JOIN iste_royalty rPaie  ON rPaie.id_royalty = r.id_royalty AND rPaie.date_paiement is not null 

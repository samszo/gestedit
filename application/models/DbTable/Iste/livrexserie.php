...
class Model_DbTable_Iste_livrexserie extends Zend_Db_Table_Abstract
{
    ...
    public function findById_livre($id_livre)
    {
        $query = $this->select()
            ...
            ->joinInner(array("s" => "iste_serie"),
                's.id_serie = ls.id_serie', array("titre"=>"CONCAT(IFNULL(s.titre_fr,' '),' / ',IFNULL(s.titre_en,' '),' / ',IFNULL(s.titre_es,' '))", "recid"=>"id_serie"))
    ...
    public function findByLivreSerie($idLivre, $idSerie)
    {
        $query = $this->select()
			...
			->setIntegrityCheck(false) //pour pouvoir sélectionner des colonnes dans une autre table
            ->joinInner(array("s" => "iste_serie"),
                'ls.id_serie = s.id_serie', array("titre"=>"CONCAT(IFNULL(s.titre_fr,' '),' / ',IFNULL(s.titre_en,' '),' / ',IFNULL(s.titre_es,' '))", "recid"=>"id_serie"))
